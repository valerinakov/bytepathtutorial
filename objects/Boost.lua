...
function Boost:draw()
    love.graphics.setColor(love.math.colorFromBytes(boost_color))
...
function Boost:die()
    ...
    self.area:addGameObject('InfoText', self.x +  table.random({-1, 1})*self.w, self.y +  table.random({-1, 1})*self.h, {text = '+BOOST', color = boost_color, w = self.w, h = self.h})
end